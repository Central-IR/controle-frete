...
app.head('/api/expedicoes', (req, res) => {
    res.status(200).end();
});

// Listar todas as expedições
app.get('/api/expedicoes', async (req, res) => {
    try {
        console.log('🔍 Buscando expedições...');
        ...
        console.log(`✅ ${data.length} expedições encontradas`);
        res.json(data || []);
    } catch (error) {
...
// Buscar expedição específica
app.get('/api/expedicoes/:id', async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('controle_frete')
            .select('*')
        ...
        res.status(500).json({ 
            error: 'Erro ao buscar expedição', 
            details: error.message 
        });
    }
});
...
app.post('/api/expedicoes', async (req, res) => {
    try {
        console.log('📝 Criando expedição:', req.body);
        ...
        if (!numero_nf || !data_expedicao || !destinatario || !cidade_destino || !transportadora || !valor_frete || !status) {
            return res.status(400).json({ 
                error: 'Campos obrigatórios faltando',
                required: ['numero_nf', 'data_expedicao', 'destinatario', 'cidade_destino', 'transportadora', 'valor_frete', 'status']
            });
        }
        ...
        const { data, error } = await supabase
            ...
            .insert([{
                ...
                volumes: volumes ? parseInt(volumes) : 1,
                status: status,
                codigo_rastreio: codigo_rastreio ? codigo_rastreio.trim() : null,
        ...
        console.log('✅ Expedição criada:', data.id);
        res.status(201).json(data);
    } catch (error) {
        console.error('❌ Erro:', error);
        res.status(500).json({ 
            error: 'Erro ao criar expedição', 
            details: error.message 
        });
    }
});
...
// Atualizar expedição
app.put('/api/expedicoes/:id', async (req, res) => {
    try {
        console.log('✏️ Atualizando expedição:', req.params.id);
        ...
        console.log('✅ Expedição atualizada');
        res.json(data);
    } catch (error) {
        console.error('❌ Erro:', error);
        res.status(500).json({ 
...
// Deletar expedição
app.delete('/api/expedicoes/:id', async (req, res) => {
    try {
        console.log('🗑️ Deletando expedição:', req.params.id);
        ...
        console.log('✅ Expedição deletada');
        res.status(204).end();
    } catch (error) {
        console.error('❌ Erro:', error);
        res.status(500).json({ 
            error: 'Erro ao excluir expedição', 
            details: error.message 
        });
    }
});
...
app.listen(PORT, '0.0.0.0', () => {
    console.log('\n🚀 ================================');
    console.log(`🚀 Servidor Controle de Frete rodando na porta ${PORT}`);
